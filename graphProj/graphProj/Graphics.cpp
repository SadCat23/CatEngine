...
#pragma region DetermineShaderPath
	...
	/////ParticleSystemAdd////
	if (!vertexSh_Particle.Init(this->device, shaderfolder + L"vertexShader_Particle.cso", layoutDepth, numElementsDepth))
	{
		return false;
	}

	if (!pixelSh_Particle.Init(this->device, shaderfolder + L"pixelShader_Particle.cso"))
	{
		return false;
	}
	D3D11_INPUT_ELEMENT_DESC layoutParticle[] =
	{
		{"POSITION", 0, DXGI_FORMAT::DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA, 0  },
		{"TEXCOORD", 0 , DXGI_FORMAT::DXGI_FORMAT_R32G32_FLOAT,0,D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA,0},
		{"COLOR", 0 , DXGI_FORMAT::DXGI_FORMAT_R32G32B32_FLOAT,0,D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_CLASSIFICATION::D3D11_INPUT_PER_VERTEX_DATA,0}
	};
	UINT numElementsParticle = ARRAYSIZE(layoutParticle);
...
bool Graphics::InitScene()
{
	...
	///add particle sysytem///////
	hr = this->m_particlePixelBuffer.Initialize(this->device.Get(), this->context.Get());
	if (FAILED(hr))
	{
		ErrorLogger::Log(hr, "Failed to create constBuffer_PartcilePixel");
		return false;
	}

	hr = this->m_particleVertexBuffer.Initialize(this->device.Get(), this->context.Get());
	if (FAILED(hr))
	{
		ErrorLogger::Log(hr, "Failed to create constBuffer_PartcileVertex");
		return false;
	}
