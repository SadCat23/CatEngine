...
bool ParticleSystem::Initialize(ID3D11Device* device, Texture* texture, ConstantBuffer<CB_VS_vertexshader_Particle>& _cb_vs_vertexshader, ConstantBuffer<CB_VS_pixelshader_Particle>& _cb_vs_pixelshader)
{
	bool result;

	this->cb_vs_vertexshader = &_cb_vs_vertexshader;
	this->cb_vs_pixelshader = &_cb_vs_pixelshader;
...
{
	return m_Texture->GetTextureResourceView();
}
...
void ParticleSystem::Draw(const XMMATRIX& viewProjectionMatrix)
{
	this->cb_vs_vertexshader->data.worldMatrix = worldMatrix;
	cb_vs_vertexshader->view = viewMatrix;
	cb_vs_vertexshader->projection = projectionMatrix;
